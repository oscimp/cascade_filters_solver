...
MinimizeArea::MinimizeArea(const std::int64_t nbStage, const double rejectionLevel, const std::string &firlsFile, const std::string &fir1File, const std::string &experimentName)
...
const std::vector<SelectedFilter> &MinimizeArea::getSelectedFilters() const {
    return m_selectedFilters;
}

void MinimizeArea::printResults(std::ostream &out) {
    m_model.update();
